...
<jsp:setProperty name="m" property="name" value='<%= request.getParameter("email") %>' />
<jsp:setProperty name="m" property="email" value='<%= request.getParameter("value") %>' />
<%
	/* 전송된 회원정보를 getParameter를 통해 가져옴 */
	// String id = request.getParameter("id");
	// String pwd = request.getParameter("pwd");
	// String name = request.getParameter("name");
	// String email = request.getParameter("email");
	
	/* 3. useBean에 전송된 회원 정보를 설정합니다. */
	// m.setId(id);
	// m.setPwd(pwd);
	// m.setName(name);
	// m.setEmail(email);
